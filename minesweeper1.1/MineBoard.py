...
class MineBoard():
    def __init__(self, row, col, minenum):
        self.__row = row
        self.__col = col
        self.__minenum = minenum#雷的数量
        self.__mboa = array([[0 for i in range(col)] for j in range(row)])#存储雷的位置的表
        self.__nboa = array([[0 for i in range(col)] for j in range(row)])#存储其他位置的表
        self.__boa = array([[0 for i in range(col)] for j in range(row)])#棋盘总表
        self.reset()
    ...
    def setvalue(self, i, j):
        if self.__mboa[i][j] == -1:#该点为雷，则无需计算
            return 0
        if i - 1 < 0:
            if j - 1 < 0:#该点位于左上角
                return -(self.__mboa[i][j + 1] + self.__mboa[i + 1][j] + self.__mboa[i + 1][j + 1])
            elif j - 1 >= 0 and j + 1 < self.__col:#最上列
            ...
            elif j - 1 >= 0 and j + 1 < self.__col:#内层
            ...
            elif j - 1 >= 0 and j + 1 < self.__col:#最下列
